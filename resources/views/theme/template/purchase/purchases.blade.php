    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-no-wrap items-center mt-2">
            <a href="/purchases/create"
               class="button text-white bg-theme-1 shadow-md mr-2 font-bold font-caps text-xs">@lang('purchase.add_new')</a>
                    ...
                    @foreach ($purchases as $purchase)
                        ...
                        <tr>
                            ...
                            <td class="font-medium text-gray-800">
                                @if($purchase->distributor != null)
                                    {{$purchase->distributor->name_ge }} <br>
                                @endif
                                <small class="text-gray-600 font-normal">@lang('purchase.dept')
                                    : {{number_format(($purchase->getPrice() - $purchase->paid)/100, 2)}}</small>
                            </td>
                            ...
                            <td class="text-center whitespace-no-wrap font-normal">
                                <div class="flex items-center justify-center w-full">
                                    ...
                                    @if($purchase->products()->count() == 0)
                                        <a href="{{route('RemovePurchase', $purchase->id)}}"
                                           class="p-2 bg-gray-300 rounded-lg ml-2">
                ...
                <form action="" method="GET">
                    <div class="flex flex-wrap -mx-3 mb-1">
                        <div class="w-full md:w-1/2 px-3 mb-4 md:mb-0">
                            ...
                            <input @if(isset($queries['code'])) value="{{$queries['code']}}"
                                   @endif class="appearance-none block w-full bg-gray-200 text-gray-700 border rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2">
                        <div class="col-span-1">
                            <div class="flex items-center justify-center font-normal text-xs">
                                <input type="checkbox" @if (isset($queries['dept'])) checked @endif id="dept"
                                       name="dept">
                                <label for="dept" class="ml-2">@lang('purchase.dept')</label>
                            </div>
                        </div>
                        <div class="col-span-1 pl-3">
                            <input data-daterange="true" @if (isset($queries['date'])) value="{{$queries['date']}}"
                                   @endif name="date" class="datepicker input text-xs w-full border block mx-auto">
                        </div>
                    ...
                    <div class="flex">
                        ...
                        <a href="{{url()->current()}}"
                           class="w-1/4 mt-2 block appearance-none flex items-center justify-center font-bold font-caps bg-red-500 text-xs text-white bg-gray-200 border border-gray-200  py-3 px-4  rounded leading-tight">
                            <svg width="1.3em" height="1.3em" viewBox="0 0 16 16" class="bi bi-x-circle"
                                 fill="currentColor" xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd"
