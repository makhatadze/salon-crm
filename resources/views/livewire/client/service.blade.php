<div>
    ...
    <h6 class="w-full font-bold mb-2 font-caps text-xs">სერვისები  <small class="font-normal ml-1">( {{$client->clientservices()->where('status', 1)->count()}} )</small></h6>
    <div class="p-2 bg-gray-200 flex items-center justify-between">
        <input wire:model="search" type="text" class="bg-gray-200 font-normal text-xs w-full focus:outline-none" placeholder="მოძებნეთ სერვისი..">
        ...
        <div class="p-2 mt-2 bg-gray-200 flex items-center justify-center">
            ...
            <label for="consignationserv" class="font-normal text-xs ml-2">კონსიგნაცია</label>
        </div>
        @if ($clientservices)
