<div>
   ...
   <div class="w-full p-2 bg-white flex items-center my-1">
       <input type="text" class="w-full font-normal text-xs focus:outline-none" wire:model="search" placeholder="#ნომერი">
   ...
   <div class="p-1 flex items-center">
       ...
       <label for="dept" class="font-normal text-xs ml-2">დავალიანება</label>
   </div>
   ...
   <a href="/purchases/edit/{{$purchase->id}}" class="my-2 @if($purchase->getPrice() != $purchase->paid) border-l-2 border-red-400 pl-2 @endif zoom-in flex items-center justify-between">
       ...
       @if ($purchase->getPrice() != $purchase->paid)
           <div>
               ...
               <small class="font-normal">დავალიანება</small>
           </div>
       @endif
       <div>
           <h6 class="font-bolder text-xs text-black">{{$purchase->distributor->{'name_'.app()->getLocale()} }}</h6>
