<div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4">
        <div class="col-span-1 py-2">
            <div class="bg-white p-3 text-left">
                <h6 class="font-bold text-xs">თარიღი <small>[დან]</small></h6>
        ...
        <div class="col-span-1 py-2">
            <div class="bg-white p-3 text-left">
                <h6 class="font-bold text-xs">თარიღი <small>[მდე]</small></h6>
        ...
        <div class="col-span-1 ml-2 py-2">
            <div class="bg-white p-3">
                <h6 class="font-bold text-xs">ტიპი</h6>
                <div class="relative w-56">
                    <select wire:model="type" class="font-normal text-xs appearance-none block w-56 mt-1 bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
                      <option>ყველა</option>
                      <option value="full">დაფარული</option>
                      <option value="consignation">ვალით</option>
                    </select>
    ...
    <div class="w-full mt-2 py-3 px-4 bg-white flex items-center">
        <div class="w-full md:w-1/4">
            <h6 class="font-medium text-xs">კლიენტი</h6>
        </div>
        <div class="w-full md:w-1/4">
            <h6 class="font-medium text-xs">თარიღი</h6>
        </div>
        ...
        <div class="w-full md:w-1/4 flex items-center justify-center">
            <h6 class="font-medium text-xs">ხელფასზე</h6>
        </div>
    </div>
   @if ($clientservices)
    ...
    <a @if ($item->sale) href="{{route('showSale', $item->sale_id)}}" @else  href="{{route('showService', $item->service_id)}}"  @endif class="w-full mt-2 py-3 px-4 bg-white flex items-center">
        <div class="w-full md:w-1/4">
                @if ($item->sale)
                <h2 class="font-bolder text-xs text-gray-700">{{$item->sale->client->{'full_name_'.app()->getLocale()} }}</h2>
                ...
                @else
                <h2 class="font-bolder text-xs text-gray-700">{{$item->service->clinetserviceable->{'full_name_'.app()->getLocale()} }}</h2>
                <div class="flex items-center">
                    ...
                    <h6 class="ml-1 font-medium font-caps text-gray-600" style="font-size: 0.65rem">{{$item->service->clinetserviceable->number}}</h6>
                </div>
                @endif
            </div>
            <div class="w-full md:w-1/4">
                @if ($item->sale)
                <h6 class="font-bolder text-xs text-gray-700">შესყიდვის თარიღი</h6>
                <span class=" font-medium font-caps text-gray-600" style="font-size: 0.65rem">{{$item->created_at}}</span>
                @else 
                <h6 class="font-bolder text-xs text-gray-700">დაჯავშნის თარიღი</h6>
                <span class=" font-medium font-caps text-gray-600" style="font-size: 0.65rem">{{$item->service->created_at}}</span>
                @endif
            </div>
            <div class="w-full md:w-1/4 flex items-center justify-center">
                <div>
